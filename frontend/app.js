            ...
            <tr data-linea="${l.id}">
              ...
              <td>${l.product_id ? `
                <div data-impuestos-chips="${l.id}" data-seleccionados="${encodeURIComponent(JSON.stringify(l.impuesto_nombres || []))}" style="display:flex;flex-wrap:nowrap;gap:.2rem">
                  ...
                  ${(l.impuesto_nombres || []).filter(n => !IMPUESTOS_RAPIDOS_NOMBRES.includes(n)).map(nombre => `<button type="button" class="btn btn-primary" style="font-size:.72rem;padding:.1rem .35rem;white-space:nowrap;flex-shrink:0" data-impuesto-chip="${l.id}" data-impuesto-nombre="${nombre.replace(/"/g, '&quot;')}">${nombre}</button>`).join('')}
                </div>
    ...
    async function toggleImpuestoLinea(lineaId, nombre) {
      ...
      try {
        ...
        const rapidosHtml = IMPUESTOS_RAPIDOS.map(({ nombre, corto }) => {
          ...
          return `<button type="button" class="btn ${activo ? 'btn-primary' : ''}" style="font-size:.72rem;padding:.1rem .35rem;white-space:nowrap;flex-shrink:0" title="${nombre}" data-impuesto-chip="${lineaId}" data-impuesto-nombre="${nombre.replace(/"/g, '&quot;')}">${corto}</button>`;
        }).join('');
        const extrasHtml = extras.map(n => `<button type="button" class="btn btn-primary" style="font-size:.72rem;padding:.1rem .35rem;white-space:nowrap;flex-shrink:0" data-impuesto-chip="${lineaId}" data-impuesto-nombre="${n.replace(/"/g, '&quot;')}">${n}</button>`).join('');
